...
if ($is_manager && isset($_GET['id'])) {
    ...
    /* --- 2. CHECK IF CAR IS IN ACTIVE AUCTION OR UNPAID CLOSED AUCTION --- */
    // Logic: Lock if status is 'Active' OR (status is 'Closed' AND winner_paid is 0)
    $auction_check_sql = "SELECT id FROM auctions 
                          WHERE car_id = ? 
                          AND (status = 'Active' OR (status = 'Closed' AND winner_paid = 0))";
    $auction_check_stmt = $conn->prepare($auction_check_sql);
    $auction_check_stmt->execute([$car_id]);
    $is_locked_by_auction = $auction_check_stmt->fetch();
...
<head>
    <meta charset="UTF-8">
    <title>Manager View - <?php echo $car['brand']; ?> | Rev Nation</title>
                        ...
                        <div class="mt-3 p-3 rounded reply-box <?php echo (!empty($r['company_reply'])) ? 'active-reply' : ''; ?>">
                            <?php if (!empty($r['company_reply']) && !$is_editing) { ?>
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <small class="fw-bold text-primary">COMPANY RESPONSE</small>
                                    <div>
            ...
            <div class="col-lg-5">
                <div class="control-panel card shadow-sm p-4">
                    ...
                    <div class="d-grid gap-3 mt-4">
                        <?php if ($is_locked_by_auction) { ?>
                            <div class="alert alert-danger mb-0 border-0 shadow-sm">
                                <h6 class="fw-bold mb-1"><i class="fa fa-lock"></i> Listing Locked</h6>
                                <p class="small mb-0">This car is currently tied to an <strong>Active or Unpaid Auction</strong>. Edit features are disabled.</p>
                            </div>

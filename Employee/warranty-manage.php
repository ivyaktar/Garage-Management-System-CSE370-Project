...
// Allow both Managers and Employees to access and manage warranties
if (isset($_SESSION['user_id']) && 
    ...
    /* --- 1. FETCH PRODUCT WARRANTY CLAIMS --- */
    // Only where car_id is NULL in the orders table
    ...
    /* --- 2. FETCH CAR WARRANTY CLAIMS --- */
    // Only where car_id is NOT NULL in the orders table
...
<head>
    <meta charset="UTF-8">
    <title>Manage Warranties - Staff Portal</title>
...
<body class="bg-light">
    ...
    <div class="container mt-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3>Warranty Claims Management</h3>
            <span class="badge bg-secondary p-2">Logged in as: <?php echo $_SESSION['role']; ?></span>
        </div>
                    ...
                    <tr class="align-middle">
                        ...
                        <td>
                            <?php if ($c['status'] == 'Pending') { ?>
                                <a href="req/warranty-status.php?id=<?php echo $c['id']; ?>&status=Approved" 
                                   class="btn btn-sm btn-success">Approve</a>
                                <a href="req/warranty-status.php?id=<?php echo $c['id']; ?>&status=Rejected" 
        ...
        <div class="table-responsive shadow-sm bg-white p-3 rounded mb-5">
            <table class="table table-hover">
                ...
                <tbody>
                    <?php 
                    $c_count = count($car_claims);
                    if ($c_count > 0) {
                        for ($j = 0; $j < $c_count; $j = $j + 1) { 
                            $cc = $car_claims[$j];
                    ?>
                    <tr class="align-middle">
                        ...
                        <td>
                            <?php if ($cc['status'] == 'Pending') { ?>
                                <a href="req/warranty-status.php?id=<?php echo $cc['id']; ?>&status=Approved" 
                                   class="btn btn-sm btn-success">Approve</a>
                                <a href="req/warranty-status.php?id=<?php echo $cc['id']; ?>&status=Rejected" 
                                   class="btn btn-sm btn-danger">Reject</a>
                            <?php } else { ?>
                                <span class="text-muted small">Processed</span>
                            <?php } ?>

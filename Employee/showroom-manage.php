...
if (isset($_SESSION['user_id']) && 
    ...
    // 3. Fetch Cars with Wishlist Count and Auction Status (Updated Logic)
    $sql = "SELECT c.*, cat.category_name,
            (SELECT COUNT(*) FROM wishlist w WHERE w.product_id = c.id) as wishlist_count,
            (SELECT COUNT(*) FROM auctions a 
             WHERE a.car_id = c.id 
             AND (a.status = 'Active' OR (a.status = 'Closed' AND a.winner_paid = 0))) as in_auction
            FROM cars c 
            ...
            <div class="col-md-3">
                <div class="filter-section shadow-sm mb-4">
                    <form action="showroom-manage.php" method="GET">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="cat_id" class="form-select form-select-sm">
                                ...
                                <?php for ($j = 0; $j < count($categories); $j = $j + 1) { ?>
                                    <option value="<?php echo $categories[$j]['id']; ?>" <?php echo (isset($_GET['cat_id']) && $_GET['cat_id'] == $categories[$j]['id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="mb-3">
                            <label class="form-label small text-muted">Year Range</label>
                            <div class="input-group input-group-sm">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="trans" class="form-select form-select-sm">
                                ...
                                <?php for ($k = 0; $k < count($transmissions); $k = $k + 1) { ?>
                                    <option value="<?php echo $transmissions[$k]; ?>" <?php echo (isset($_GET['trans']) && $_GET['trans'] == $transmissions[$k]) ? 'selected' : ''; ?>>
                                        <?php echo $transmissions[$k]; ?>
                                    </option>
                                <?php } ?>
                            </select>
                        </div>

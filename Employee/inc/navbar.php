<?php 
// Check if employee data is already fetched, if not, fetch it for the navbar name
if (!isset($employee) && isset($_SESSION['user_id'])) {
    ...
    $nav_sql = "SELECT fname FROM employees WHERE user_id = ?";
    
    $nav_stmt = $conn->prepare($nav_sql);
    
    $nav_stmt->execute([$nav_user_id]);
    
    $employee_data = $nav_stmt->fetch();
    
    if ($employee_data) {
        $nav_display_name = $employee_data['fname'];
    } else {
        $nav_display_name = "Employee";
    }

} else {
    
    if (isset($employee['fname'])) {
        $nav_display_name = $employee['fname'];
    } else {
        $nav_display_name = "Employee";
    }
...
// Fetch count of unread inquiries for the badge
      ...
      <ul class="navbar-nav ms-auto ms-auto-custom mb-2 mb-lg-0 align-items-center">
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle text-white fw-bold" href="#" id="userDrop" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            <i class="fa fa-user-circle-o fa-lg me-1 text-success"></i> <?php echo $nav_display_name; ?>
          </a>
